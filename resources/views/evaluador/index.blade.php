                                    ...
                                    @forelse ($postulantes as $postulante)
                                        <tr>
                                            ...
                                            <td class="border px-4 py-2">
                                                <a href="{{ route('evaluador.editExpediente', $postulante) }}" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-3 rounded">{{ __('Calificar Expediente') }}</a>
                                            </td>
                                            <td class="border px-3 py-2">
                                                @php
                                                $nota_parcial1 = null; // Valor predeterminado
                                                @endphp

                                                @foreach ($postulante->preinscripcion as $preinscripcion)
                                                    @foreach ($preinscripcion->postulantes as $postulante1)
                                                        @foreach ($postulante1->puntajes as $puntaje)
                                                            @php
                                                            $nota_parcial1 = $puntaje->nota_parcial1 ?? $nota_parcial1; // Si $puntaje->nota_parcial1 no es nulo, lo asignamos a $nota_parcial1
                                                            @endphp
                                                        @endforeach
                                                    @endforeach
                                                @endforeach

                                                {{ $nota_parcial1 ?? 0 }} <!-- Mostramos $nota_parcial1 o 0 si es nulo -->
                                            </td>
                                            <td class="border px-4 py-2">
                                            ...
                                            <td class="border px-3 py-2">
                                                ...
                                                @foreach ($postulante->preinscripcion as $preinscripcion)
                                                    @foreach ($preinscripcion->postulantes as $postulante1)
                                                        @foreach ($postulante1->puntajes as $puntaje)
                                                            @php
                                                            $nota_parcial2 = $puntaje->nota_parcial2 ?? $nota_parcial2; // Si $puntaje->nota_parcial1 no es nulo, lo asignamos a $nota_parcial1
                                                            @endphp
                                                        @endforeach
                                                    @endforeach
                                                @endforeach
                                                    ...
                                                    @foreach ($postulante->preinscripcion as $preinscripcion)
                                                        @foreach ($preinscripcion->postulantes as $postulante1)
                                                            @foreach ($postulante1->puntajes as $puntaje)
                                                                ...
                                                                {{ $puntaje->nota_total }}  <!--Si $puntaje->nota_parcial1 no es nulo, lo asignamos a $nota_parcial1-->
