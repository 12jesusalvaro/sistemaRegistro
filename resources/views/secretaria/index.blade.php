                ...
                <div class=" shadow p-6 bg-gray-50 dark:bg-zinc-800">
                    <div class="grid grid-cols-8 gap-8">
                        <div class="col-span-6">
                            <h1 class="text-2xl font-bold text-black dark:text-white">{{ __('Lista de Preinscritos') }}</h1>
                        </div>
                        ...
                        <div class="col-span-2">
                            <select id="convocatorias" name="convocatorias" class="mt-1 block w-full py-1 px-2 border border-gray-300 bg-white dark:bg-zinc-800 dark:text-white rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                                @foreach ($convocatorias->reverse() as $convocatoria)
                                    <option value="{{ $convocatoria->id }}" {{ $loop->first ? 'selected' : '' }}>
                                        {{ $convocatoria->nombre }}
                                    </option>
                                @endforeach
                            </select>
...
<script>
    $(document).ready(function() {
        ...
        var convocatoriaIdInicial = $('#convocatorias').val();
        console.log('Convocatoria inicial:', convocatoriaIdInicial);
        cargarPostulantes(convocatoriaIdInicial);
        ...
        function cargarPostulantes(convocatoriaId) {
            $.ajax({
                url: '/obtener-preinscritos/' + convocatoriaId,
                type: 'GET',
        ...
        function cargarResultadosDeBusqueda() {
            var formData = new FormData();
            var convocatoriaID = $('#convocatorias').val();
            console.log(convocatoriaID);
            // Asegúrate de crear un nuevo objeto FormData si no lo has hecho ya
