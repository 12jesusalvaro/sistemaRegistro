...
class ContadorController extends Controller
{
    public function index(Request $request)
    {
        ...
        */
        ...
        $contador = Auth::user()->contador;
        $mencionContador = $contador->mencion;
        $ultimaConvocatoria = Convocatoria::latest()->first();
        // Consulta para obtener todos los postulantes que tienen la misma mención que contador y que hayan registrado su pago
        $query = User::role('Postulante')
            ->whereHas('preinscripcion', function ($query) use ($mencionContador, $ultimaConvocatoria) {
                $query->where('mencion_id', $mencionContador->id);
                $query->where('convocatoria_id', $ultimaConvocatoria->id);
            })
            ->whereHas('preinscripcion.pagoInscripcion', function ($query) {
        ...
        return view('contador.index', compact('postulantes'));
    }
